  ...
  case SM_STATE_DATA_RECEIVED:
    {
      ...
      humValue  = pRxBuff[6];
      /**> Compute Pressure Valie */
      pressValue = (pRxBuff[7] << 8) + (pRxBuff[8] & 0xFF);

      UART_PRINT("Received from Node #%d\r\n", xRxFrame.Cmdtag);
      UART_PRINT("RSSI dBm %.1f\r\n", rRSSIValue);
      UART_PRINT("TEMP %d °C\r\n", tempValue);
      UART_PRINT("HUM %d %%\r\n", humValue);
      UART_PRINT("PRESS %d mBar\r\n", pressValue);
